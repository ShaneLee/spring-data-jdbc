 ...
 * @since 2.4
 */
class SimilarToVisitor extends FilteredSubtreeVisitor {

	private final SimilarTo similarTo;
	private final RenderContext context;
	private final RenderTarget target;
	private final StringBuilder part = new StringBuilder();
	private @Nullable PartRenderer current;

	SimilarToVisitor(SimilarTo condition, RenderContext context, RenderTarget target) {
		super(it -> it == condition);
		this.similarTo = condition;
	...
	Delegation leaveNested(Visitable segment) {
		...
		if (current != null) {
			if (part.length() != 0) {
				...
				if (similarTo.isNegated()) {
					part.append("NOT ");
				}

				part.append("SIMILAR TO ");
			}
